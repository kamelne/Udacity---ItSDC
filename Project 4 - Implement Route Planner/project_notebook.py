...
def get_current_node(self):
    ...
    for node in self.openSet:
        if testing_node== None:
            testing_node = node
            min_node = self.calculate_fscore(node)
        if self.calculate_fscore(node) < min_node:
            testing_node = node
            min_node = self.calculate_fscore(node)
    return testing_node
...
def record_best_path_to(self, current, neighbor):
    ...
    self.fScore[current] = self.calculate_fscore(neighbor)
...
# **ANSWER**:
# 
# A* finds the shortest path from a start point to a goal while checking the cost of each step taken and the estimated cost to reach the goal if that step to taken.

# ---
# How does A-Star search algorithm differ from Uniform cost search? What about Best First search?
# 
# **ANSWER**:
# Uniform cost finds the shortest path and search only looks at the cost per path taken and does not account for the estimate of distance from the start to the goal, thus expands in all directions from start until the goal is found.
# 
# Best First takes into account the estimate distance from start to goal but then only works in that direction to shorten the distance to the goal, but if there is an obstacle in the way it will not back track to find another way it will continue moving closer to the goal.
# 
# A* uses both of these finding the shortest path while keeping in account the distance to the goal. 
...
# **ANSWER**:
# 
# A heuristic is a quick problem solving approach is not optimal but will work without spending more time to final what might be an impossible solution. In A* it is the function used to determing the stright line distance from one node to another. 
...
# **ANSWER**:
# 
# Consistent heuristic is when the function always estimates less than or equal to the cost of reaching the goal from a neighboring node plus the cost of reaching that node
